...
MsgBox "基準日：" & baseDate & " より前のデータを移行" & vbCrLf & "完済データは " & tgtDbPath & " に移動します。", vbOKOnly, "年次更新"
...
if DBCopyF = True then
  logFile.WriteLine "DBコピー直後のため、全データDBのデータ削除はスキップしました。"
Else
  ' DBDA010_借入金_TEMP
  sql = ""
...
' -------------------------
' INSERT（全データ.TEMPテーブルに追加）
' -------------------------
if DBCopyF = True then
  logFile.WriteLine "DBコピー直後のため、TEMPテーブルへのデータ追加はスキップしました。"
Else
  ' DBDA010_借入金
...
' -------------------------
' INSERT（差分追加をTEMPテーブルから追加）
' -------------------------
if DBCopyF = True then
  logFile.WriteLine "DBコピー直後のため、差分追加のTEMPテーブルからのデータ追加はスキップしました。"
Else
  ' DBDA010_借入金
...
' -------------------------
' INSERT（借入金以外のテーブル 全データに追加）
' -------------------------
if DBCopyF = True then
  logFile.WriteLine "DBコピー直後のため、借入金以外のテーブルの全データへのデータ追加はスキップしました。"
Else
  ' DBDA010_銀行マスタ
  sql = ""
...
logFile.WriteLine "テーブル [" & TBK & "] から " & recordsAffected & " 件削除しました。（完済データ）"  
...
connSrc.Execute sql, recordsAffected
logFile.WriteLine "テーブル [" & TBKTR & "] から " & recordsAffected & " 件削除しました。（完済データ関連）"
...
sql = sql & " K.借入番号 IS NULL;"
connSrc.Execute sql, recordsAffected
logFile.WriteLine "テーブル [" & TBKTR2 & "] から " & recordsAffected & " 件削除しました。（完済データ関連）"
...
sql = sql & " K.借入番号 IS NULL;"
connSrc.Execute sql, recordsAffected
logFile.WriteLine "テーブル [" & TBKU1 & "] から " & recordsAffected & " 件削除しました。（完済データ関連）"
...
sql = sql & " K.借入番号 IS NULL;"
connSrc.Execute sql, recordsAffected
logFile.WriteLine "テーブル [" & TBKU2 & "] から " & recordsAffected & " 件削除しました。（完済データ関連）"
