...
Option Explicit
'
Private Const pPROGRAM_ID As String = "frm_R借入金明細表"
'
...
'
    '社債フラグ
    wIndex = 0
    For j = 1 To UBound(p借入番号)
        If p借入番号(j).借入番号 = P8.FCStr(借入番号) Then
            wIndex = j
                Exit For
        End If
    Next
'
    ...
    '              レポート表示
    ' =========================================
    If p借入番号(wIndex).社債フラグ = 0 Then
        GRpt.帳票名 = "借入明細表"
        Set rpt = New RDA010_借入明細表
    ElseIf p借入番号(wIndex).社債フラグ = 1 Then
        If p借入番号(wIndex).手入力区分 = 0 Then
            GRpt.帳票名 = "社債明細表"
            Set rpt = New RDA010_借入明細表
        ElseIf p借入番号(wIndex).手入力区分 = 1 Then
            GRpt.帳票名 = "社債明細表"
            Set rpt = New RDA010_社債明細表
            
        ElseIf p借入番号(wIndex).手入力区分 = 2 Then
            
                MsgBox "未完成データです", vbExclamation
                Call CEkey.SetFs(借入番号, True)
