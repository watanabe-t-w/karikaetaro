...
Begin VB.Form frm_M金利シミュレーショングループマスタ 
   BorderStyle     =   3  '固定ﾀﾞｲｱﾛｸﾞ
   Caption         =   "金利シミュレーショングループマスタ"
   ClientHeight    =   6285
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   6930
   Icon            =   "frm_M金利シミュレーショングループマスタ.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   ...
   End
End
Attribute VB_Name = "frm_M金利シミュレーショングループマスタ"
...
Option Explicit
'
Private Const pPROGRAM_ID As String = "金利ｼﾐｭﾚｰｼｮﾝGPマスタ"
...
'
    ...
    wstr = ""
    wstr = wstr & "SELECT"
    wstr = wstr & " IIF(取消フラグ=0 OR 取消フラグ IS NULL,'','*') AS Grd削除,"
    wstr = wstr & " 金利グループ区分 AS Grd金利グループ区分,"
    wstr = wstr & " 金利グループ名 AS Grd金利グループ名"
    wstr = wstr & " FROM DAAA115_金利シミュレーショングループ"
    wstr = wstr & " WHERE (0=0)"
    If Me.削除データを表示.Value = 0 Then
        wstr = wstr & " AND (取消フラグ = 0 OR 取消フラグ IS NULL)"
    End If
    ...
    Call XZMA010_DataGrid_Init
        ...
        End If
        Call XZMA010_DataGrid_Set("金利グループ区分", "グループ", 1400, "L")
        Call XZMA010_DataGrid_Set("金利グループ名", "名称", 3000, "L")
...
'
    On Error Resume Next
        ...
        wCheckValue = Adodc1.Recordset.Fields.Item("Grd金利グループ区分")
        If Err.Number = 3021 Then GoTo Exit_Sub
    On Error GoTo Err_Hundle
        金利グループ区分 = P8.FCStr(Adodc1.Recordset.Fields.Item("Grd金利グループ区分"))
    On Error GoTo 0
...
'
    ...
    '                パラメータ
    ' =========================================
    p金利グループ区分 = P8.FCStr(金利グループ区分.Text)
    ...
    wstr = wstr & "* "
    wstr = wstr & "FROM DAAA115_金利シミュレーショングループ "
    wstr = wstr & "WHERE 金利グループ区分 = '" & p金利グループ区分 & "'"
    ...
    '変更登録
        削除.Enabled = True
        新規変更.Caption = "変更"
        Me.金利グループ名.Text = P8.FCStr(wRs("金利グループ名"))
...
'
    ...
    DoEvents
    Call XZMA010_DataGrid_Bookmark(DataGrid1, Adodc1, "Grd金利グループ区分 = '" + 金利グループ区分 + "'")
'
    Exit Function
'
'----------< ERROR ROUTINE >---------------------------------------------------
...
'
    ...
    '               更新行を表示
    '----------------------------------------
    Call XZMA010_DataGrid_Bookmark(DataGrid1, Adodc1, "Grd金利グループ区分 = '" + w金利グループ区分 + "'")
    Call CEkey.SetFs(Me.金利グループ区分, True)
'
End Sub
...
'
    ...
    '               パラメータセット
    ' =========================================
    p金利グループ区分 = P8.FCStr(Me.金利グループ区分.Text)
    p金利グループ名 = P8.FCStr(Me.金利グループ名.Text)
'
    ' =========================================
    '               入力チェック
    ' =========================================
    If p金利グループ区分 = "" Then
        MsgBox "グループが未入力です。", vbExclamation
        Call CEkey.SetFs(金利グループ区分, True)
        Exit Sub
    End If
    
    If Len(p金利グループ区分) <> 2 Then
        MsgBox "グループは2桁で入力してください。", vbExclamation
        Call CEkey.SetFs(金利グループ区分, True)
        Exit Sub
    End If
...
'
    ...
    '新規登録
        ...
        Call AdoRecordsetOpen(GDb, wRs, wstr)
            ...
            wRs("金利グループ区分") = p金利グループ区分
            wRs("金利グループ名") = p金利グループ名
            wRs("取消フラグ") = 0
...
'
    ...
    ElseIf 新規変更.Caption = "変更" And 削除.Value = 1 Then
        wslog = "削除"
    End If
    GLogStr = "金利ｼﾐｭﾚｰｼｮﾝGP=" & p金利グループ区分 & ","
    GLogStr = GLogStr & "名称=" & p金利グループ名
...
Private Sub 金利グループ区分_LostFocus()
