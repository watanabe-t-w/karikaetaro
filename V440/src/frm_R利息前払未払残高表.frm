...
Begin VB.Form frm_R利息前払未払残高表 
   ...
   End
End
Attribute VB_Name = "frm_R利息前払未払残高表"
...
Option Explicit
'
Private Const pPROGRAM_ID As String = "frm_R利息前払未払残高表"
'
...
Private Sub 実行日2_LostFocus()
    ...
    '和暦
        Call P8.FCControlLeft(実行日2, 2)
    Else
    '西暦
        Call P8.FCControlLeft(実行日2, 4)
    End If
End Sub
...
'
    ...
    Dim wsShube As String, wsBumon As String, wsGinko As String, wsKinyu As String
...
'
    ...
    Call MFA001_GRrptBunrui("frm_R利息前払未払残高表")
'
    入力チェック = True
'
    Exit Function
...
'
    ...
    '帳票出力条件 決算用のみ　帳票処理後に戻す
    G基本情報.借入金管理区分 = XMXA020_区分("借入金管理区分", "決算用")
    GRpt.借入金管理区分 = G基本情報.借入金管理区分
'
    ' =========================================
    '           　 ボタン制御
    ' =========================================
...
'
    ...
    '              レポート表示
    ' =========================================
    Set rpt = New RDF010_利息前払未払残高表
    '杉村倉庫仕様
    'Set rpt = New RDF011_利息前払未払残高表
'
    rpt.Show vbModal
'
    Set rpt = Nothing
...
'
    ...
    ' =========================================
    For j = 0 To 3
        If GRpt.S_利息 = "分類" & CStr(j + 1) Then
            wsBunrui(j) = "利息区分"
        ElseIf GRpt.S_種別 = "分類" & CStr(j + 1) Then
            wsBunrui(j) = "借入金種別"
