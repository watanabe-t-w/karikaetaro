VERSION 5.00
Begin {9EB8768B-CDFA-44DF-8F3E-857A8405E1DB} RDA010_利息前払未払明細表 
   Caption         =   "利息明細表"
   ClientHeight    =   8010
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   13155
   Icon            =   "RDA010_利息前払未払明細表.dsx":0000
   StartUpPosition =   3  'Windows の既定値
   WindowState     =   2  '最大化
   _ExtentX        =   23204
   _ExtentY        =   14129
   SectionData     =   "RDA010_利息前払未払明細表.dsx":0ECA
End
Attribute VB_Name = "RDA010_利息前払未払明細表"
...
Dim wiCont As Integer
Dim wd利息残高 As Double
'
'------------------------------------------------
...
'
    ...
    wstr = wstr & "Format(KM.締年月,'" & Gfmt年月 & "') As GrpFld_M,"
    'wstr = wstr & "Format(KM.返済年月日,'" & Gfmt年月日 & "') As GrpFld_D,"
    wstr = wstr & "Format(KM.返済年月日,'" & Gfmt年月日 & "')&'-'&KM.月毎NO As GrpFld_D," '内入対応
    wstr = wstr & "Format(KM.締年月,'" & Gfmt年月 & "') As I_年月,"
    wstr = wstr & "Format(KM.返済年月日,'" & Gfmt年月日 & "') As I_年月日,"
    wstr = wstr & "KM.月毎NO As I_番号,"
    wstr = wstr & "KM.利息計算対象額 As I_利息計算対象額,"
    wstr = wstr & "KM.利息額増 As I_利息額増,"
    wstr = wstr & "KM.利息額減 As I_利息額減,"
    wstr = wstr & "KM.日割日数 As I_日数,"
    
    '2014/08/26 計算式修正
    wstr = wstr & "KM.利息期間対象額 As I_利息期間対象額,"
    wstr = wstr & "KM.利息期間対象日数 As I_利息期間対象日数,"
    wstr = wstr & "KM.利息調整F As I_利息調整F,"
    ...
    'wstr = wstr & " Order BY KM.返済年月日,KM.利息額増 desc,KM.月毎NO"
...
'
    ...
    '利息先払 1：減、2：増
        If P8.FCDbl(Me.Detail.Controls("I_利息額減")) <> 0 Then
            ws番号1 = P8.FCDbl(Me.Detail.Controls("I_番号"))
            ws年月日1 = P8.FCStr(Me.Detail.Controls("I_年月日"))
            wd利息計算対象額1 = P8.FCDbl(Me.Detail.Controls("I_利息計算対象額"))
            wd利息額1 = P8.FCDbl(Me.Detail.Controls("I_利息額減"))
            ws期間1 = P8.FCStr(Me.Detail.Controls("I_開始日")) & "～" & P8.FCStr(Me.Detail.Controls("I_終了日"))
            wi日数1 = P8.FCDbl(Me.Detail.Controls("I_日数"))
            ws利率1 = P8.FCDbl(Me.Detail.Controls("I_利率"))
            
            '2014/08/26 計算式修正
            wd利息期間対象額1 = P8.FCDbl(Me.Detail.Controls("I_利息期間対象額"))
            wi利息期間対象日数1 = P8.FCDbl(Me.Detail.Controls("I_利息期間対象日数"))
            wi利息調整F1 = P8.FCDbl(Me.Detail.Controls("I_利息調整F"))
            If wi利息調整F1 = 0 Then
                wd引算1 = wd引算1 + wd利息額1
            End If
        End If
        ...
        If P8.FCDbl(Me.Detail.Controls("I_利息額増")) <> 0 Then
            ws番号2 = P8.FCDbl(Me.Detail.Controls("I_番号"))
            ws年月日2 = P8.FCStr(Me.Detail.Controls("I_年月日"))
            wd利息計算対象額2 = P8.FCDbl(Me.Detail.Controls("I_利息計算対象額"))
            wd利息額2 = P8.FCDbl(Me.Detail.Controls("I_利息額増"))
            ws期間2 = P8.FCStr(Me.Detail.Controls("I_開始日")) & "～" & P8.FCStr(Me.Detail.Controls("I_終了日"))
...
'
    ...
    If wd利息額1 <> 0 Then
        ...
        '解約 利息先払
            ...
            Me.GroupFooter1.Controls("G10_式1") = "=" & Format(wd前月前払利息残高, "#,##0") & " + （" & Format(wd利息期間対象額1, "#,##0") & "）"
            wd引算1 = 0
        Else
            Me.GroupFooter1.Controls("G10_式1") = "= " & Format(wd利息期間対象額1, "#,##0") & " × " & Me.GroupFooter1.Controls("G10_日数1") & " / " & wi利息期間対象日数1
        End If
...
'
    If wd利息額2 <> 0 Then
        ...
        Me.GroupFooter1.Controls("G10_番号2") = wiCont
        Me.GroupFooter1.Controls("G10_年月日2") = ws年月日2
        Me.GroupFooter1.Controls("G10_利息額2") = Format(wd利息額2, "#,##0")
        Me.GroupFooter1.Controls("G10_期間2") = ws期間2
        Me.GroupFooter1.Controls("G10_日数2") = wi日数2
        Me.GroupFooter1.Controls("G10_利率2") = Format(ws利率2, "#,##0.00000")
        Me.GroupFooter1.Controls("G10_式2") = "= " & Format(wd利息計算対象額2, "#,##0") & " × " & Me.GroupFooter1.Controls("G10_利率2") & "%" & " × " & Me.GroupFooter1.Controls("G10_日数2") & " / " & ws金利年間日数
    
        Me.GroupFooter1.Controls("sh2").Visible = True
        wiCont = wiCont + 1
    Else
    ...
    End If
'
    Me.GroupFooter1.Controls("LG10_U1").Caption = ""
    If ws番号1 <> "0" And ws番号1 <> "" Then
        Me.GroupFooter1.Controls("LG10_U1").Caption = "*"
    End If
    
    Me.GroupFooter1.Controls("LG10_U2").Caption = ""
    If ws番号2 <> "0" And ws番号2 <> "" Then
        Me.GroupFooter1.Controls("LG10_U2").Caption = "*"
    End If
'
End Sub
...
'
    ...
    wd01 = wd利息残高 + P8.FCDbl(Me.GroupFooter2.Controls("G20_利息額増")) - P8.FCDbl(Me.GroupFooter2.Controls("G20_利息額減"))
    Me.GroupFooter2.Controls("G20_利息残高") = Format(wd01, "#,##0")
