...
Option Explicit
'
Private Const pPROGRAM_ID As String = "部門マスタ"
...
'
    ...
    wstr = wstr & " 部門番号 AS Grd部門番号,"
    wstr = wstr & " 部門名 AS Grd部門名,"
    wstr = wstr & " 部門略名 AS Grd部門略名"
    wstr = wstr & " FROM DAAA200_部門マスタ"
    wstr = wstr & " WHERE (0=0)"
    ...
    Call XZMA010_DataGrid_Init
        ...
        End If
        Call XZMA010_DataGrid_Set("部門番号", "コード", 1400, "L")
        Call XZMA010_DataGrid_Set("部門名", "部門名", 3200, "L")
        Call XZMA010_DataGrid_Set("部門略名", "部門略名", 1400, "L")
...
'
    On Error Resume Next
        ...
        wCheckValue = Adodc1.Recordset.Fields.Item("Grd部門番号")
        If Err.Number = 3021 Then GoTo Exit_Sub
    On Error GoTo Err_Hundle
        部門番号 = P8.FCStr(Adodc1.Recordset.Fields.Item("Grd部門番号"))
    On Error GoTo 0
...
'
    ...
    '変更登録
        削除.Enabled = True
        新規変更.Caption = "変更"
        Me.部門名.Text = P8.FCStr(wRs("部門名"))
        Me.部門略名.Text = P8.FCStr(wRs("部門略名"))
...
'
    ...
    DoEvents
    Call XZMA010_DataGrid_Bookmark(DataGrid1, Adodc1, "Grd部門番号 = '" + 部門番号 + "'")
'
    Exit Function
'
'----------< ERROR ROUTINE >---------------------------------------------------
...
'
    ...
    '               更新行を表示
    '----------------------------------------
    Call XZMA010_DataGrid_Bookmark(DataGrid1, Adodc1, "Grd部門番号 = '" + w部門番号 + "'")
    Call CEkey.SetFs(Me.部門番号, True)
'
End Sub

Private Sub 部門番号_LostFocus()
    Call P8.FCControlLeft(部門番号, 10)
    Call 画面セット(False)
End Sub

Private Sub 部門名_LostFocus()
    Call P8.FCControlLeft(部門名, 50)
End Sub

Private Sub 部門略名_LostFocus()
    Call P8.FCControlLeft(部門略名, 10)
End Sub
...
'
    ...
    '               パラメータセット
    ' =========================================
    p部門番号 = P8.FCStr(Me.部門番号.Text)
    p部門名 = P8.FCStr(Me.部門名.Text)
    p部門略名 = P8.FCStr(Me.部門略名.Text)
'
    ' =========================================
    '               入力チェック
    ' =========================================
    If p部門番号 = "" Then
        MsgBox "部門コードが未入力です。", vbExclamation
        Call CEkey.SetFs(部門番号, True)
        Exit Sub
    End If
...
'
    ...
    End If
    GLogStr = "部門番号=" & p部門番号 & ","
    GLogStr = GLogStr & "部門名=" & p部門名
    GLogStr = GLogStr & "部門略名=" & p部門略名
